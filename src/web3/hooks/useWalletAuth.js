...
export const useWalletAuth = (onLoginSuccess) => {
    ...
    // Watch for signed data to call the callback
    useEffect(() => {
        if (signedData && signedData.address && signedData.signature && onLoginSuccess) {
            onLoginSuccess(signedData.address, signedData.signature);
            setSignedData(null);
            cleanupSigningState();
        }
    }, [signedData, onLoginSuccess]);
