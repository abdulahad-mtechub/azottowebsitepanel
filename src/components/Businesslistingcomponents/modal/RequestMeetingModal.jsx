...
const RequestMeetingModal = ({
  ...
  const [getUserDetails, { data }] = useLazyQuery(ME);

  useEffect(() => {
    if (userId && visible) {
      getUserDetails({ variables: { getUserDetailsId: userId } });
    }
  }, [userId, visible]);

  const user = data?.getUserDetails;
  const [acceptEnda] = useMutation(CREATE_ENDA);
  const [businessMeeting, { loading: meetingLoading }] =
    useMutation(BUSINESS_MEETING);
  ...
  const checkAllTerms = () => {
    ...
    const allChecked =
      values.ndaAgree && values.termsAgree && values.commissionAgree;
  ...
  const steps = [
    {
      ...
      content: (
        <SignJusoorEndaStep
          ...
          onClose={onClose}
          user={user}
          onCheckboxChange={checkAllTerms}
        />
      ),
    },
    {
    ...
    >
      ...
      <Flex gap={10} justify="end">
        ...
        {current === steps.length - 1 && (
          <Button
            type="primary"
            aria-labelledby="Send meeting request"
            className="btn bg-brand"
            onClick={async () => {
              try {
                const values = await form.validateFields();
