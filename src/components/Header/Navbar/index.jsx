  ...
  const profileItems = [
    {
      ...
      label: (
        <Flex align="center" gap={8}>
          <Text strong>{walletAddress?.slice(0, 6)}...{walletAddress?.slice(-4)}</Text>
        </Flex>
      ),
    },
    {
                ...
                return (
                  <List.Item key={item?.id ?? index} className={itemClassName}>
                    <List.Item.Meta
                      ...
                      title={
                        <Text
                          className={`notification-title ${item?.isRead ? "read" : "unread"
                            }`}
                        >
                          {titleText}
                        </Text>
