...
const FinalDealsStep = ({ inprogressdeal }) => {
  ...
  const documents = uploadDocs.filter((doc) => {
    ...
    return keywords.some((keyword) => docTitle.includes(keyword.toLowerCase()));
  });
      ...
      <Row gutter={[16, 24]}>
        <Col span={24}>
          <Flex vertical gap={0} className="mb-3">
            <Text className="fw-600 fs-14">
              {t("Document Received Confirmation")}
            </Text>
            <Text className="fs-13 text-gray">
              {t(
                "The seller has uploaded final documents. Please review and confirm you’ve received all required legal materials before the deal is finalized."
              )}
            </Text>
          </Flex>
