...
const SellerSingleInprogressSteps = ({ deal }) => {
  ...
  const isCommissionVerified =
    deal?.isCommissionVerified || deal?.status === "COMMISSION_VERIFIED";
  const isStep1Completed = deal?.isDsaSeller && deal?.isDsaBuyer;
  const isStep2Completed =
    deal?.bankAccountId ||
    deal?.status === "SELLER_PAYMENT_VERIFICATION_PENDING" ||
  ...
  const steps = [
    ...
    },
    {
      ...
      status:
        deal?.isPaymentVedifiedSeller && !deal?.isDocVedifiedBuyer
            ...
            {openPanels.includes(item.key) ? (
              <Flex align="center" gap={5}>
                ...
                item.status.toLowerCase()?.includes("waiting") ? (
                  <Text className="sendstatus badge-cs fs-10 sm-pill fw-500 fit-content">
                    {item?.status}
                  </Text>
                ) : (
                  <Text className="success badge-cs fs-10 sm-pill fw-500 fit-content">
                    {item?.status}
                  </Text>
                )}
