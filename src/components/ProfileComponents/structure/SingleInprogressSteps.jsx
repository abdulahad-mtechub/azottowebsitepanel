...
const SingleInprogressSteps = ({ inprogressdeal }) => {
  ...
  const [activeBank, { data: activeBankData }] =
    useLazyQuery(GETUSERACTIVEBANK);

  useEffect(() => {
    if (inprogressdeal?.sellerId) {
      activeBank({
        variables: { getUserActiveBanksId: inprogressdeal?.sellerId },
      });
    }
  ...
  const isStep2Completed =
    inprogressdeal?.isDsaSeller && inprogressdeal?.isDsaBuyer; // Both signed DSA
  ...
  const [openPanels, setOpenPanels] = useState(
    inprogressdeal
      ? Array.from({ length: initialStep + 1 }, (_, i) => (i + 1).toString())
      : ["1"]
  );
  ...
  const steps = [
    ...
    },
    {
      ...
      content: (
        <DigitalSaleAgreementStep form={form} details={inprogressdeal} />
      ),
      status:
    ...
    },
    {
      ...
      content: (
        <PayBusinessAmountstep
          inprogressdeal={inprogressdeal}
          bank={activeBankData?.getUserActiveBanks}
          form={form}
        />
      ),
      status: inprogressdeal?.isPaymentVedifiedSeller
        ? t("Verified")
        : t("Pending"),
      lockedTitle: t("DSA Required"),
      lockedDesc: t(
        "Waiting for seller & buyer to sign the Digital Sale Agreement."
      ),
  ...
  const stepItems = steps.map((item) => {
    ...
    return {
          ...
          <span className="collapse-indicator">
            {openPanels.includes(item.key) ? (
              <Flex align="center" gap={5}>
                {item?.status
                  ?.toLowerCase()
                  .includes(t("pending").toLowerCase()) ||
                item?.status
                  ?.toLowerCase()
                  .includes(t("waiting").toLowerCase()) ||
                item?.status
                  ?.toLowerCase()
                  .includes(t("verification").toLowerCase()) ? (
                  <Text className="sendstatus badge-cs fs-10 fw-500 fit-content">
                    {item?.status}
                  </Text>
                ) : (
                  <Text className="success badge-cs fs-10 fw-500 fit-content">
                    {item?.status}
                  </Text>
                )}
      ...
      children: !item.isEnabled ? (
        <Flex className="text-center" vertical justify="center" align="center">
          <Typography.Title level={5} className="fw-500 m-0 fs-14">
            {item?.lockedTitle}
          </Typography.Title>
          <Text className="fs-14 text-gray">{item?.lockedDesc}</Text>
        </Flex>
      ) : (
