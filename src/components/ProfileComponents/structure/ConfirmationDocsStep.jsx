...
const ConfirmationDocsStep = ({ form, details }) => {
    ...
    const uploadDocs = [
        ...
        doc => doc.title === "Commercial Registration (CR)"
        ) && {
    ...
    <Row gutter={[16, 24]}>
        ...
        <Col span={24}  justify="space-around" align="start">
            <Flex className="w-100 ">
                <div>
                <Text className="fw-600 text-medium-gray fs-13">Commercial Registration (CR)</Text>
            <SingleFileUpload
                form={form}
                name={"uploadimge"}
                title={"Upload"}
                onUpload={(file) => handleSingleFileUpload(file, "Commercial Registration (CR)")} 
                multiple={false}
                message={message}
            />
                </div>
            <div>
            <Text className="fw-600 text-medium-gray fs-13">Notarized Ownership Transfer Letter</Text>
            <SingleFileUpload
                form={form}
                name={"uploadimge"}
                title={"Upload"}
                onUpload={(file) => handleSingleFileUpload(file, "Notarized Ownership Transfer Letter")} 
                multiple={false}
                message={message}
            />
            </div>
