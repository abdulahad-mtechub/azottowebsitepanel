...
const SellerInProgressDeals = memo(({ setInprogressDeal, searchValue }) => {
  ...
  const [fetchDeals, { data: offerDeals, loading: loadingDeals }] =
    useLazyQuery(SELLERINPROGRESSDEALS, {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status, record) => {
        ...
        return (
          <span className={`${badgeClass} fs-12 badge-cs fw-500 fit-content`}>
            {status}
          </span>
        );
      },
  ...
  const sellerofferData = useMemo(() => {
    return (
      offerDeals?.getSellerInprogressDeals?.deals?.map((deal) => ({
        key: deal.id,
...
SellerInProgressDeals.displayName = "SellerInProgressDeals";
