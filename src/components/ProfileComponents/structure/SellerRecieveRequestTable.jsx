...
const SellerRecieveRequestTable = memo(({ isBuyer, searchValue }) => {
  ...
  const sellerrecievedrequestData =
    data?.getReceivedMeetingRequests?.items?.map((meeting) => {
      return {
        key: meeting.id,
        title: meeting.business?.businessTitle,
        buyername: getNamePreview(meeting.requestedBy?.name),
  ...
  return (
    <>
      ...
      <DeleteModal
        ...
        subtitle={t(
          "Rejecting this meeting request will remove it from your request list. Are you sure you want to proceed?"
        )}
        refetch={() =>
          refetchMeetings({
            variables: {
